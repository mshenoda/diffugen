...
def generate_inpaint_dataset(dataset_config:str="dataset.json"):
    ...
    sd_model_name = "realistic_vision_inpainting"
    sd_model_path = f"../models/{sd_model_name}" # Realistic_Vision_V4.0  Realistic_Vision_V5.1_noVAE
    pipe = StableDiffusionInpaintPipeline.from_pretrained(pretrained_model_name_or_path=sd_model_path, torch_dtype=torch.float16)
    ...
    for prompt_info in prompts_list:
        ...
        sd_model_info_json_file = f"{dataset_output}/{dataset_name}/inpaint/sd_model_info.json"
